...
public class MiniMapController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (raceManager.finishAnimation)
        {
            ...
            // Проверка, найден ли playerCar
            if (playerCar != null)
            {
                target = playerCar.transform;
            }
            string targetName = target.name; // Получаем имя объекта
    ...
    public MapObject RegisterMapObject(GameObject owner, MiniMapEntity mme)
    {
        ...
        RectTransform iconRect = curMGO.GetComponent<RectTransform>();
        if (mme.priority == 1)
        {
            iconRect.SetAsLastSibling(); // Устанавливаем иконку на передний план
        }
        else
        {
            iconRect.SetAsFirstSibling(); // Ставим иконку на задний план
        }
